...
const logger = utils.getLogger('create-channel');
...
const _configtxProto = grpc.load(path.join(__dirname, 'node_modules/fabric-client/lib/protos/common/configtx.proto')).common;

let Constants = require('./constants.js');
let ClientUtils = require('./clientUtils.js');
...
function createChannel(channel_name, constants) {
    ...
    let config = null;		// Network channel configuration
    const signatures = [];		// Collect signatures to submit to orderer for channel creation

    // Attempt to create the channel as a client of Constants.IMPORTER_ORG
    let org = ORGS[Constants.UET].name;
    ...
    return Client.newDefaultKeyValueStore({		// Set the key-value store location
        ...
        let envelope_bytes = fs.readFileSync(path.join(__dirname,  Constants.channelConfig));
        config = client.extractChannelConfig(envelope_bytes);
        console.log('Successfully extracted the config update from the configtx envelope');
        const enrollmentAndSignPromises = [];
